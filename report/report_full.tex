...
Recent work in sensitive domains has also explored synthetic-only datasets generated from explicit target labels or scores, followed by manual screening for coherence and label alignment (Moëll and Aronsson, 2026). Our dataset follows the opposite choice: it uses real public web snippets rather than LLM-generated examples. This improves ecological validity for the intended corpus-filtering scenario, because the data includes real source styles and real webpage noise. However, it also makes the dataset less controllable than a synthetic design: source coverage, genre balance, and annotation consistency must be documented and checked manually.
...
The project has several limitations. First, the dataset is mainly Chinese. Dictionary entries and primary transcriptions are almost entirely Chinese or Classical Chinese, while English and Japanese scholarly discussion are not systematically represented. As a result, the model may misclassify English or Japanese academic prose as \texttt{noise}. Second, the dataset is small, with only 400 examples, so the results may depend on source style and sampling choices. Since each label is strongly associated with a small number of source types, the classifier may learn website or genre style as well as the intended textual function. Third, the annotation was manually constructed for a course-scale project, without independent double annotation or inter-annotator agreement. Future work should document source-level distributions more systematically, add independent annotation checks, expand the dataset across more websites and languages, and evaluate on external sources. Finally, the model only uses isolated snippets and does not use page structure, titles, surrounding paragraphs, or source-site information; these signals could improve future versions.
...
\begin{itemize}
  \item Moëll, B. and Aronsson, F. S. ``High-accuracy prediction of mental health scores from English BERT embeddings trained on LLM-generated synthetic self-reports: a synthetic-only method development study.'' \textit{Frontiers in Digital Health} (2026). \url{https://doi.org/10.3389/fdgth.2025.1694464}
